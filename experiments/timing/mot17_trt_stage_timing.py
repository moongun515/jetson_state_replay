...
# This file is intended to be stored at:
# ~/jetson_state_replay/experiments/timing/mot17_trt_stage_timing.py
...
OUT_PATH = OUT_DIR / "mot17_02_yolo11n_trt_fp16_stage_timing_state.json"

METRICS_DIR = ROOT / "outputs" / "metrics" / "final"
CSV_PATH = METRICS_DIR / "mot17_02_yolo11n_trt_fp16_stage_timing.csv"
SUMMARY_PATH = METRICS_DIR / "mot17_02_yolo11n_trt_fp16_stage_timing_summary.json"

MODEL_NAME = "yolo11n.engine"
TARGET_CLASS_NAMES = {"person"}
CONF_THRESHOLD = 0.15

# Use None for all 600 frames. Set 60 for a quick smoke test.
MAX_FRAMES = None
...
def summarize_rows(rows):
    ...
    return {
        ...
        "avg_read_ms": mean_or_zero([row["read_ms"] for row in rows]),
        ...
        "correlation_with_active_track_count": {
            "read_ms": pearson_corr(
                [row["active_track_count"] for row in rows],
                [row["read_ms"] for row in rows],
            ),
...
def convert_yolo_bytetrack_to_state():
    ...
    for idx, img_path in enumerate(image_paths, start=1):
        ...
        row = {
            ...
            "read_ms": round(read_elapsed * 1000, 3),
        ...
        if idx % 50 == 0:
            print(
                f"[INFO] {idx:>3}/{len(image_paths)} | "
                f"objects={row['active_track_count']:>2} | "
                f"read={row['read_ms']:>7.3f} ms | "
    ...
    state = {
        ...
        "model_info": {
            ...
            "note": (
                "TensorRT FP16 YOLO detection + ByteTrack tracking. "
                "Stage timing experiment."
            ),
        },
        "conversion_summary": {
    ...
    summary = {
        "experiment": "mot17_02_yolo11n_trt_fp16_stage_timing",
        "model": MODEL_NAME,
        "tracker": "bytetrack.yaml",
    ...
    print(
        f"[INFO] Processing FPS    : "
        f"{len(frames) / processing_elapsed_sec:.3f}"
    )
    ...
    print(
        f"- FPS from avg : "
        f"{steady_summary['estimated_fps_from_avg_total_ms']}"
    )
