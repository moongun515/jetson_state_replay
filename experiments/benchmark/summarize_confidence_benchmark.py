...
METRIC_DIR = ROOT / "outputs/metrics/benchmark/confidence"
STATE_DIR = ROOT / "outputs/state_json/benchmark/confidence"
...
def parse_state(path: Path) -> dict[str, int]:
    data: Any = json.loads(path.read_text(encoding="utf-8"))
    frames = extract_frames(data)

    total_objects = 0
    track_ids: set[int | str] = set()
...
def prefix(tag: str, run_idx: int) -> str:
    return (
        f"mot17_02_yolo11n_trt_fp16_imgsz960_{tag}"
        f"_bytetrack_queue4_maxn_super_run{run_idx}"
    )


rows: list[dict[str, float | int | str]] = []

for tag, confidence in [
    ("conf015", 0.15),
    ("conf025", 0.25),
]:
    for run_idx in [1, 2, 3]:
        name = prefix(tag, run_idx)
        ...
        row = {
            ...
            "avg_target_detection_count": float(
                steady["avg_target_detection_count"]
            ),
            "total_objects": state["total_objects"],
            "unique_track_ids": state["unique_track_ids"],
            "gt_ratio": state["total_objects"] / GT_TOTAL_OBJECTS,
            **resources,
        }
...
print("===== CONFIDENCE RUN RESULTS =====")

print(
    f"{'Conf':>6s} {'Run':>4s} "
    f"{'Pipe FPS':>9s} {'Cons FPS':>9s} "
    f"{'Infer+Track':>12s} {'Objects':>9s} "
    f"{'Tracks':>7s} {'GT Ratio':>9s} "
    f"{'Avg Temp':>9s} {'Power W':>8s}"
)
...
average_rows: list[dict[str, float]] = []

for confidence in [0.15, 0.25]:
    group = [
        row
        for row in rows
    ...
    pipeline_values = [
        float(row["pipeline_fps"])
        for row in group
    ]

    consumer_values = [
        float(row["consumer_fps"])
        for row in group
    ]
    ...
    average_row = {
        ...
        "inference_tracking_ms_avg": mean(
            float(row["inference_tracking_ms"])
            for row in group
        ),
        "state_build_ms_avg": mean(
            float(row["state_build_ms"])
            for row in group
        ),
        "consumer_total_ms_avg": mean(
            float(row["consumer_total_ms"])
            for row in group
        ),
        "objects_avg": mean(
            float(row["total_objects"])
            for row in group
        ),
        "tracks_avg": mean(
            float(row["unique_track_ids"])
            for row in group
        ),
        "gt_ratio_avg": mean(
            float(row["gt_ratio"])
        ...
        "avg_power_w": mean(
            float(row["avg_power_mw"])
            for row in group
        ) / 1000,
        "max_power_w": max(
            float(row["max_power_mw"])
            for row in group
        ) / 1000,
        "avg_ram_mb": mean(
            float(row["avg_ram_mb"])
            for row in group
        ),
    }
...
csv_path = (
    METRIC_DIR /
    "confidence_benchmark_imgsz960_three_run_average.csv"
)
