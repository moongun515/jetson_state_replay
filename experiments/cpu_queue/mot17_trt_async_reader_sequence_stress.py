...
SEQUENCE_NAME = os.environ.get("MOT_SEQUENCE", "MOT17-02-DPM").strip()

SEQ_DIR = ROOT / "data" / SEQUENCE_NAME
SEQINFO_PATH = SEQ_DIR / "seqinfo.ini"
IMG_DIR = SEQ_DIR / "img1"

OUT_DIR = ROOT / "outputs" / "state_json" / "cpu_queue"
CLASS_MODE = os.environ.get("STRESS_CLASSES", "all").strip().lower()

if CLASS_MODE == "person":
    TARGET_CLASS_NAMES = {"person"}
elif CLASS_MODE == "traffic":
    TARGET_CLASS_NAMES = {
    ...
    raise ValueError(
        "STRESS_CLASSES must be one of: person, traffic, all"
    )

RUN_TAG = f"{SEQUENCE_NAME.lower().replace('-', '_')}_yolo11n_trt_fp16_async_queue4_stress_v3_{CLASS_MODE}"

OUT_PATH = OUT_DIR / f"{RUN_TAG}_state.json"

METRICS_DIR = ROOT / "outputs" / "metrics" / "cpu_queue"
CSV_PATH = METRICS_DIR / f"{RUN_TAG}_timing.csv"
SUMMARY_PATH = METRICS_DIR / f"{RUN_TAG}_summary.json"
...
def resolve_inference_class_ids(model):
    """Return YOLO class IDs passed into model.track().

    Important:
    - person mode: detector and tracker only process person
    - traffic mode: detector and tracker process person + traffic classes
    - all mode: no class restriction
    """
    if TARGET_CLASS_NAMES is None:
        return None

    name_to_id = {name: int(class_id) for class_id, name in model.names.items()}
    missing = sorted(name for name in TARGET_CLASS_NAMES if name not in name_to_id)
    if missing:
        raise ValueError(f"Classes not found in model.names: {missing}")

    return sorted(name_to_id[name] for name in TARGET_CLASS_NAMES)
...
def convert_yolo_bytetrack_to_state():
    ...
    inference_class_ids = resolve_inference_class_ids(model)

    print(f"[INFO] Model loaded in {initialization_sec:.3f} sec")
    print(f"[INFO] Inference classes: {inference_class_ids if inference_class_ids is not None else 'ALL'}")
        ...
        if results and len(results) > 0:
            ...
            if boxes is not None:
                ...
                for box in boxes:
                    ...
                    if TARGET_CLASS_NAMES is not None and cls_name not in TARGET_CLASS_NAMES:
                        continue
    ...
    state = {
        ...
        "model_info": {
            ...
            "target_class_names": ("ALL_CLASSES" if TARGET_CLASS_NAMES is None else sorted(list(TARGET_CLASS_NAMES))),
            "queue_size": QUEUE_SIZE,
            "inference_class_ids": inference_class_ids,
    ...
    summary = {
        "experiment": f"{SEQUENCE_NAME.lower().replace('-', '_')}_yolo11n_trt_fp16_async_queue4_stress_v3_{CLASS_MODE}",
        "model": MODEL_NAME,
        "tracker": "bytetrack.yaml",
